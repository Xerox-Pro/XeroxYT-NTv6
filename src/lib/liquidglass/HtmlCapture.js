 ...
 */
export function invalidateFontEmbedCache() {
...
function filterFontBlocksForElement(blocks, element) {
    ...
    return blocks.filter((block) => {
        ...
        if (block.unicodeRanges) {
            const hasMatch = matchingUsages.some((u) => u.text.length > 0
...
async function buildFontBlocks() {
    ...
    //    EmbeddedFontBlock.
    const embedded = await Promise.all(candidates.map(async (ruleText) => {
    ...
    if (embedded.length === 0) {
        console.warn('LiquidGlass: no @font-face rules found on the page; '
...
export class HtmlCapture {
    ...
    async captureElement(element, force = false) {
        ...
        if (this._capturing.has(element))
            return;
    ...
    async captureToCanvas(element, cssW, cssH, hideNodes = null) {
        if (cssW <= 0 || cssH <= 0)
            return null;
        const hideSet = hideNodes && hideNodes.length
            ? new Set(hideNodes)
            : null;
        try {
...
//# sourceMappingURL=HtmlCapture.js.map