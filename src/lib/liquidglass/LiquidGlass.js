...
export class LiquidGlass {
    ...
    constructor({ root, glassElements, defaults = {} }) {
         ...
         * the set.
         */
        this._userMarkedChanged = new Set();
         ...
         */
        this._glassContentDirty = new Set();
        this._fpsFrames = 0;
        this._fpsTime = 0;
        this._observer = null;
        ...
        if (!root)
            throw new Error('LiquidGlass: `root` element is required.');
        this.root = root;
    ...
    _markGlassAndDependents(element, rectOverride) {
        ...
        const elementBox = this._getPixelRect(elementDOMRect, rootRect, dpr, this.glassSet.has(element) ? SHADOW_PAD : 0);
        ...
        for (const child of this._sortedChildren) {
            ...
            if (!this.glassSet.has(child))
                continue;
            const sampleRect = this._getSampleRect(child.getBoundingClientRect(), rootRect, dpr);
    ...
    _markGlassesIntersecting(element) {
        const rootRect = this.root.getBoundingClientRect();
        const dpr = window.devicePixelRatio || 1;
        const elementBox = this._getPixelRect(element.getBoundingClientRect(), rootRect, dpr, this.glassSet.has(element) ? SHADOW_PAD : 0);
        for (const glass of this.glassSet) {
            const sampleRect = this._getSampleRect(glass.getBoundingClientRect(), rootRect, dpr);
            if (LiquidGlass._rectsIntersect(elementBox, sampleRect)) {
    ...
    async _captureGlassContent(targets = null) {
        ...
        try {
            for (const [el, glassCanvas] of this.glassCanvases) {
                if (targets && !targets.has(el))
                    continue;
                const rect = el.getBoundingClientRect();
                const img = await this.capture.captureToCanvas(el, rect.width, rect.height, [glassCanvas]);
                if (img) {
    ...
    _getSortedChildren() {
        ...
        const isFlexOrGridParent = rootDisplay === 'flex' || rootDisplay === 'inline-flex' ||
            rootDisplay === 'grid' || rootDisplay === 'inline-grid';
        const tagged = children.map((el, domIndex) => {
            const style = window.getComputedStyle(el);
            const hasStackingContext = LiquidGlass._formsStackingContext(style, isFlexOrGridParent);
    ...
    static _formsStackingContext(style, isFlexOrGridParent) {
        if (style.position !== 'static')
            return true;
        if (isFlexOrGridParent && style.zIndex !== 'auto')
            return true;
    ...
    _handlePointerDown(e) {
        ...
        for (let i = this._sortedChildren.length - 1; i >= 0; i--) {
            ...
            if (e.clientX >= visualLeft && e.clientX <= visualLeft + elW &&
    ...
    _handlePointerMove(e) {
        ...
        if (posLeft < margin)
            newTx += margin - posLeft;
        if (posTop < margin)
            newTy += margin - posTop;
        if (posLeft > maxLeft)
            newTx -= posLeft - maxLeft;
        if (posTop > maxTop)
            newTy -= posTop - maxTop;
    ...
    _renderFrame() {
        ...
        for (const child of this._sortedChildren) {
            if (!this.glassSet.has(child))
                continue;
            this._renderGlassElement(child, rootRect, dpr, isDragging, dirtyTargets, renderedThisFrame);
        }
    }
    /**
    ...
    _renderGlassElement(child, rootRect, dpr, isDragging, dirtyTargets, renderedThisFrame) {
        ...
        if (needsShaderRender && glassCanvas) {
            ...
            this.renderer.uploadAndBlur(this._sceneCanvas, 0, 0, sampleRect.w, sampleRect.h, config.blurAmount);
            this.renderer.clear();
            ...
            ctx.drawImage(this.renderer.canvas, 0, 0, glassCanvas.width, glassCanvas.height, 0, 0, glassCanvas.width, glassCanvas.height);
    ...
    _glassHasDynamicContributors(currentGlass, sampleRect, rootRect, dpr) {
        ...
        if (this._childHasDynamicContent(currentGlass))
            return true;
        for (const child of this._sortedChildren) {
            if (child === currentGlass)
                break;
            if (this.glassSet.has(child))
                continue;
            if (!this._childHasDynamicContent(child))
                continue;
    ...
    _drawNonGlassChildToScene(child, sampleRect, rootRect, dpr) {
        ...
        this.capture.drawCachedElement(child, this._sceneCtx, rect.x - sampleRect.x, rect.y - sampleRect.y, rect.w, rect.h);
    ...
    _drawMediaElement(el, targetCtx, sampleRect, rootRect, dpr) {
        ...
        if (tag === 'CANVAS') {
            ...
            if (!img.complete || img.naturalWidth === 0)
                return false;
            this._drawMediaFitted(targetCtx, img, img.naturalWidth, img.naturalHeight, el, r, dx, dy, dw, dh);
            return true;
        }
        else if (tag === 'VIDEO') {
            const vid = el;
            ...
            if (vid.readyState < 1)
                return false;
            try {
                this._drawMediaFitted(targetCtx, vid, vid.videoWidth, vid.videoHeight, el, r, dx, dy, dw, dh);
            }
            catch {
    ...
    _drawPriorGlassToScene(child, sampleRect, rootRect, dpr) {
        ...
        if (glassCanvas) {
            ...
            if (LiquidGlass._rectsIntersect(shaderRect, sampleRect)) {
                this._sceneCtx.drawImage(glassCanvas, 0, 0, glassCanvas.width, glassCanvas.height, shaderRect.x - sampleRect.x, shaderRect.y - sampleRect.y, shaderRect.w, shaderRect.h);
            }
        }
        ...
        if (!LiquidGlass._rectsIntersect(contentRect, sampleRect))
            return;
        this._sceneCtx.drawImage(contentImg, contentRect.x - sampleRect.x, contentRect.y - sampleRect.y, contentRect.w, contentRect.h);
    }
    _getSampleRect(elRect, rootRect, dpr) {
    ...
    _getPaintOverflowPad(element) {
        ...
        const maskImage = style.maskImage
            || style.webkitMaskImage;
        const paintsOutsideBounds = (style.boxShadow && style.boxShadow !== 'none')
